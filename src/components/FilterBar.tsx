        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-dark-bg border border-dark-border rounded-xl px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-primary-500 transition-colors min-h-[40px]"
            >
              {Object.entries(sortLabels).map(([key, label]) => (
                <optgroup key={key} label={label}>
                  <option value={`${key}-desc`}>{label} 从高到低</option>
                  <option value={`${key}-asc`}>{label} 从低到高</option>
                </optgroup>
              ))}
            </select>
          </div>
